...
if calculated_loan <= 0:
    ...
    if is_under_one_year:
        ...
        # 💡 [지침 변경 반영] 월할이 아닌 정확한 재직 '일수(Days)'를 구하여 일할 계산 연환산 체계 구축
        ...
        with col_salary_input:
            # 💡 [지침 변경 반영] 명세표/임금대장 직인 날인 서류 기준 문구 구체화
        ...
        with col_bonus_input:
            ...
            total_bonus = 0 # 일할 연환산 단일화 처리를 위해 내부 변수 0 가드
            
        # 💡 [지침 변경 반영] 공사 실제 지침 공식: (누적 과세대상 총소득 / 총 재직일수) * 365일 = 연환산 소득
